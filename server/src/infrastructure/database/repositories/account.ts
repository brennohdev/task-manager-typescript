...
export class AccountRepository implements IAccountRepository {
  async create(account: Account, session?: unknown): Promise<Account> {
    if (!Types.ObjectId.isValid(account.userId)) {
      throw new Error('Invalid userId passed to AccountRepository.create()');
    }
  ...
  async updateRefreshToken(
    ...
    await AccountModel.findOneAndUpdate({ providerId }, { refreshToken, tokenExpiry });
  }
